# Never build Python3 bindings by default
%bcond_with python3
%bcond_with python36
...
Source0:        %{url}/archive/%{lib_name}-%{version}/%{lib_name}-%{version}.tar.gz
...
set -ex
%autosetup -p1 -n %{lib_name}-%{lib_name}-%{version}
...
%license COPYING
%doc README.md
%exclude %{_libdir}/%{lib_name}.so.*
%{python2_sitearch}/%{lib_name}/
%{python2_sitearch}/%{lib_name}-%{version}-py%{python2_version}.egg-info
